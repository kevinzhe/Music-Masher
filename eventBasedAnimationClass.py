...
class EventBasedAnimationClass(object):
    ...
    def __init__(self, width=300, height=300, root = None, canvas = None):
        self.root = root
        self.canvas = canvas
        self.width = width
        self.height = height
        self.timerDelay = 1000 # in milliseconds (set to None to turn off timer)
    ...
    def run(self, root = None, canvas = None):
        print "running!"
        # create the root and the canvas
        if root == None and canvas == None:
            self.root = Tk()
